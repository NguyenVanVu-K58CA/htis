...
void VehicleCouting::process()
{
  ...
  for(std::map<cvb::CvID,cvb::CvTrack*>::iterator it = tracks.begin() ; it != tracks.end(); it++)
  {
    ...
    */
	...
	if (track->active == 1){
	...
	else if (track->active == 2){
		bbb = centroid.x;
		tout();
		ccc = abs(bbb - aaa);
		std::ofstream fj("distance.txt");
		fj << ccc;
		fj.close();

		dem++;

		std::fstream myfile1("tin.txt", std::ios_base::in);
		int a;
		myfile1 >> a;
		myfile1.close();

		std::fstream myfile2("tout.txt", std::ios_base::in);
		int b;
		myfile2 >> b;
		myfile2.close();
		...
		std::cout << "interval : " << cc << "\n";

		cc *= 0.000000001;
		std::cout << "interval after: " << cc << "\n";

		std::fstream myfile3("distance.txt", std::ios_base::in);
		int s;
		myfile3 >> s;
		myfile3.close();
  ...
  if(showAB == 0 && dem != 0)
  {
	  ...
	  std::cout << " Aneverage of speed is : " << sumOfSpeed / dem << " km/h \n";
	//  std::cout << " dem  = " << dem << "\n";
	  std::cout << " Number of vehicle is : " << countAB << "\n";
